...
function atxMain(){
	...
	log("" + symnum.toString() + " initsymbol");
...
function store(type, mode, ref){
	...
	if(typeof w === "number")
		w = w.toString();
	log("\n storing token at " + tp + " --- " + type + " | " + w + "|" + mode);
	tp++;
}
...
function readM(){
	...
	switch(w0){
		case '@' : storew(w, "IND"); readR(); break;
		case '+' : storew(w, "INX"); readR(); break;
...
function gencode(){
	...
	log("\ngencode [op=" + op.toString() + " , mode=" +  mode + "] ");
...
function pass1(){
	...
	for(var i = 0; i < lines.length && !EoF; i++){
	    ...
	    while(true) {
	    	...
	    	log(" [" + w + ", " + (t+1).toString() + "/" + tokens.length.toString() + "] ");
	    	if(cmp(w, ".SYMBOL")){
		    	state = 'S';
		    }else if(cmp(w, ".DATA")){
		    	loc = parseInt(tok());
		    	 ...
		    	 switch(state){
		    	 	case 'S':
		    	 		if(typeof symTable[w] !== 'undefined')
		    	 			...
		    	 			log("new symbol stored : " + w + " = " + symTable[w][0].toString());
		    	 		}
		    	 		break;
		    	 	case 'D':
		    	 		if(isNumeric(w))
		    	 	...
		    	 	case 'C':
		    	 		if(w.charAt(0) == ':'){
		    	 			...
		    	 			if(typeof symTable[w] !== 'undefined')
		    	 				...
		    	 				log("new label stored : " + w + " = " + loc.toString());
		    	 			}
		    	 		}
		    	 		else if(w != ''){
		    	 			var er = false;
		...
		switch(mem[tp][0]){
			case "DOTC":
				...
				while(mem[tp][0] != "DOTC" && mem[tp][0] != "DOTD" && mem[tp][0] != "DOTE"){
					log("\n C generating code block from tp = " + tp.toString() + " | " + mem[tp][0] + "|" + mem[tp][1] + "|" + mem[tp][2]);
					gencode();
					loc++;
				}
				break;
			case "DOTD":
				...
				tp++;
				while(mem[tp][0] != "DOTC" && mem[tp][0] != "DOTD" && mem[tp][0] != "DOTE"){
					log("\n D generating data block from tp = " + tp.toString() + " | " + mem[tp][0] + "|" + mem[tp][1] + "|" + mem[tp][2]);
